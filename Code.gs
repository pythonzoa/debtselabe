...
/**
 * 시트에서 관리자 목록 및 필터 설정 가져오기
 * @returns {Object} { admins: string[], allowedFilters: string[] }
 */
 ...
 * @returns {Object} { errors: Array, warnings: Array, isValid: boolean }
 */
...
/**
 * [핵심] 관리자가 '발행' 버튼을 누를 때 실행 (원본 -> 스냅샷 복사)
 * @returns {Object} { success: boolean, timestamp: string, validation: Object }
 */
...
/**
 * [핵심] 화면에 데이터를 제공하는 함수 (페이지네이션 지원)
 * @param {number} startRow - 시작 행 (0부터 시작)
 * @param {number} maxRows - 최대 행 수
 * @returns {Object} 데이터 및 메타정보
 */
function getData(startRow, maxRows) {
  ...
  try {
    ...
    // 데이터 범위 계산
    const dataStartRow = headerRowIndex + 2; // 1-based index
...
/**
 * 허용 필터 추가
 * @param {string} filterName - 필터 컬럼명
 * @returns {Object} 처리 결과
 */
